...
const maybeThrowTypeError = (type, registry) => {
    if (!registry[type] || typeof registry[type] !== 'object') {
        throw new Error(`prosemirror-compare registry does not know how to compare ${type}`);
    }
}
...
const innerDiff = (oldVersion, newVersion, registry) => {
    ...
    const compare = (oldVersion, newVersion, context) => {
        ...
        return registry[oldVersion.type].compare(oldVersion, newVersion, context);
    }
    ...
    return registry.doc.compare(
        oldVersion,
        newVersion,
        {add, remove, compare, weight, incomparable},
    );
}

export const diff = (oldVersion, newVersion, registry = buildRegistry(baseRegistry)) => {
